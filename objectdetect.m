...
% 
% 
% TemplatePoints = detectSURFFeatures(bwTemplate);
% ImagePoints = detectSURFFeatures(bwImage);
% 
% figure;
% imshow(bwTemplate);
% title('100 Strongest Feature Points from Template Image');
% hold on;
% plot(selectStrongest(bwTemplate, 100));
% 
% figure;
% imshow(bwImage);
% title('100 Strongest Feature Points from Image');
% hold on;
% plot(selectStrongest(bwImage, 100));
...
[boxFeatures, TemplatePoints] = extractFeatures(bwTemplate, TemplatePoints);
[sceneFeatures, ImagePoints] = extractFeatures(bwImage, ImagePoints);

squarePairs = matchFeatures(boxFeatures, sceneFeatures);

matchedBoxPoints = TemplatePoints(squarePairs(:, 1), :);
matchedScenePoints = ImagePoints(squarePairs(:, 2), :);

[tform, inlierBoxPoints, inlierScenePoints] = ...
    estimateGeometricTransform(matchedBoxPoints, matchedScenePoints, 'affine');

TemplatePolygon = [1, 1;...                           % top-left
        size(bwTemplate, 2), 1;...                 % top-right
        size(bwTemplate, 2), size(bwTemplate, 1);... % bottom-right
        1, size(bwTemplate, 1);...                 % bottom-left
        1, 1];                   % top-left again to close the polygon

newTemplatePolygon = transformPointsForward(tform, TemplatePolygon);
% Displays the detected Template
figure;
imshow(Image);
hold on;
line(newTemplatePolygon(:, 1), newTemplatePolygon(:, 2), 'Color', 'y');
title('Detected Box');